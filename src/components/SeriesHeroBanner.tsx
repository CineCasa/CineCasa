...
import { Star, Calendar, PlaySquare, Layers } from 'lucide-react';
import { useSeriesHero } from '@/hooks/useSeriesHero';
...
export function SeriesHeroBanner() {
  const { currentItem, isLoading } = useSeriesHero();
        ...
        >
          <motion.img
            ...
            onError={(e) => {
              ...
              img.src = `https://placehold.co/1920x1080/1a1a2e/4a5568?text=${encodeURIComponent(currentItem.title?.charAt(0).toUpperCase() || 'S')}`;
            }}
          />
        </motion.div>
      </AnimatePresence>
        ...
        <div className="w-full max-w-7xl mx-auto px-12 xl:px-20">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              >
                <span className="px-4 py-1.5 bg-gradient-to-r from-[#00E5FF] to-[#00B8D4] text-black text-sm font-black uppercase tracking-wider rounded-lg">
                  Série
                </span>
              ...
              {/* Título com drop-shadow preto forte */}
              <motion.h1
                ...
                style={{
                  textShadow: '2px 2px 4px rgba(0,0,0,0.8), 4px 4px 20px rgba(0,0,0,0.9), 0 0 60px rgba(0,0,0,0.6)'
                }}
              >
                {currentItem.title}
              </motion.h1>

              {/* Bloco de Metadados Premium */}
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-lg">
                    {currentItem.seasons} {currentItem.seasons === 1 ? 'Temporada' : 'Temporadas'}
                  </span>
                </div>

                <span className="w-1.5 h-1.5 bg-white/40 rounded-full" />

                {/* Episódios (se disponível) */}
                {currentItem.episodes && (
                  <>
                    <div className="flex items-center gap-2">
                      <PlaySquare size={18} className="text-[#00E5FF]" />
                      <span className="font-bold text-lg">
                        {currentItem.episodes} Episódios
                      </span>
                    </div>
                    <span className="w-1.5 h-1.5 bg-white/40 rounded-full" />
                  </>
              ...
              {/* Sinopse - exatamente 2 linhas com ellipsis e drop-shadow */}
              <motion.p
              ...
              >
                ...
                <span className="text-white/40 text-sm font-medium">Próxima série em 7s</span>
              </motion.div>
            </motion.div>
          </AnimatePresence>
        </div>
