  ...
  const fetchContent = useCallback(async () => {
    ...
    try {
      ...
      const { data: cinemaData, error } = await supabase
        ...
        .or('genero.ilike.%nacional%,genero.ilike.%nacionais%,genero.ilike.%brasil%,genero.ilike.%brasileiro%,category.ilike.%nacional%,category.ilike.%nacionais%,category.ilike.%brasil%,category.ilike.%brasileiro%')
        .not('poster', 'is', null)
        .limit(100);
