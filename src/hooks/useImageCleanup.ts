...
const useImageCleanup = () => {
  ...
  }, [scanTableForInvalidImages, cleanupInvalidImages]);

  // Limpar apenas uma tabela
  const cleanupTable = useCallback(async (tableName: string): Promise<void> => {
    setIsScanning(true);
    setStats(prev => ({ ...prev, totalChecked: 0, invalidFound: 0, cleaned: 0, errors: [] }));

    try {
      console.log(`🚀 [ImageCleanup] Iniciando limpeza da tabela: ${tableName}`);
      ...
      setStats({
        totalChecked: invalidImages.length,
        invalidFound: invalidImages.length,
        cleaned,
        errors: []
      });

      console.log(`✅ [ImageCleanup] Limpeza da tabela ${tableName} concluída:`, {
        totalChecked: invalidImages.length,
        invalidFound: invalidImages.length,
        cleaned
      });
      ...
      console.error(`💥 [ImageCleanup] Erro na limpeza da tabela ${tableName}:`, error);
      setStats(prev => ({
        ...prev,
        errors: [...prev.errors, `Erro na tabela ${tableName}: ${error.message}`]
      }));
    } finally {
      setIsScanning(false);
    }
